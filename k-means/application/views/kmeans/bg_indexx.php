...
<div id="container">
	...
	<table cellpadding="5" border="1" cellspacing="0" width="100%">
		...
		$id = $this->db->query('select max(transaksi) as t from produk');
		foreach($id->result() as $i)
		{
			$SL = $i->t;
		}

		$ide = $this->db->query('select min(transaksi) as r from produk');
		foreach($ide->result() as $i)
		{
			$L = $i->r;
		}

		$KL= $SL/2;
		//definisi centroid (Pusat Cluster pertama)
		$c1a = $SL;
		// $c1b = $L;
		// $c1c = $KL;
		...
		$q = "insert into centroid_temp(IdPrd,iterasi,c1,c2,c3) values('".$s['IdPrd'].",1,'".$arr_c1[$no]."','".$arr_c2[$no]."','".$arr_c3[$no]."')";
		$this->db->query($q);
